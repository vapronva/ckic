...
func (i *Instance) UpdateConfig(configData string, method string) error {
	var adminURL string
	logger := log.With().
		Str("node", i.NodeName).
		Str("pod", i.PodName).
	...
	case "direct":
		clientset := i.KubeClient
		pod, err := clientset.CoreV1().Pods(i.Namespace).Get(context.Background(), i.PodName, metav1.GetOptions{})
		if err != nil {
			logger.Error().Err(err).Msg("Failed to get pod information")
			i.FailureCount++
			return fmt.Errorf("failed to get pod %s: %w", i.PodName, err)
		}
	...
	default:
		err := fmt.Errorf("unknown communication method: %s", method)
	...
	if err != nil {
		...
		i.FailureCount++
		return fmt.Errorf("failed to send POST to %s: %w", adminURL, err)
	}
